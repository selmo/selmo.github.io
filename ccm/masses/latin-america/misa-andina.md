# Misa Andina (Gloria)

![Region](https://img.shields.io/badge/Region-Latin_America_(Peru)-blue)
![Style](https://img.shields.io/badge/Style-Huayno-green)
![Difficulty](https://img.shields.io/badge/Difficulty-중-orange)
![Year](https://img.shields.io/badge/Year-Unknown-lightgrey)

## 개요

**작곡가**: Javier Leturia  
**출판**: OCP  
**언어**: Spanish  
**OCP 분류**: Latin flavored  
**구성**: Gloria 단일 설정(부분 설정)  
**비고**: OCP에서 “Exclusively online!”(온라인 전용)으로 소개

### 특징 (OCP 소개 요약)

- Huayno 스타일로 작곡된 Gloria 설정으로, “페루의 소리”를 환기한다고 소개됨
- 여러 음악 전통을 합성(synthesis)한 기쁜(joyous) 설정으로 안내됨

---

## 사용 근거 (Evidence)

OCP 제품 페이지의 온라인 전용 컬렉션에 수록:
- OCP 온라인 전용(Exclusively Online) 미사곡으로 분류
- Gloria 단일 설정으로 페루 huayno 전통을 전례에 적용

## 검색 키워드

`Misa Andina Gloria`, `Huayno Gloria`, `Javier Leturia`

---

## 🎧 듣기

- [YouTube에서 검색](https://www.youtube.com/results?search_query=Misa+Andina+Gloria+Javier+Leturia+Huayno)

## 참고 링크

- [OCP - Misa Andina](https://www.ocp.org/en-us/collections/dg/74)
- [OCP - Mass Settings (Index)](https://www.ocp.org/en-us/mass-settings)

---

[⬅️ Back to International Mass Index](../../InternationalMass.md)

